...
Source for `https://overssite.com` — the public landing page, DMG download host, and `/welcome` post-install page that the Oversite app's onboarding wizard auto-opens.
...
4. Add domain `overssite.com` (apex) and `www.overssite.com` (subdomain). Set the www entry to redirect to apex via the Domains UI dropdown.

## DMG download URL

`https://overssite.com/download/Oversite-latest.dmg` 302-redirects to `https://github.com/ahmedradwan210-gif/oversite-releases/releases/latest/download/Oversite_aarch64.dmg`. The `Oversite_aarch64.dmg` filename is version-stable by maintainer convention — see `PRJCTOVRSITE/RELEASES.md` for the rename-on-upload step that keeps the redirect target valid across releases.
